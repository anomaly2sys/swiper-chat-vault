...
import { Send, Shield, Lock, Eye, EyeOff, Settings, Users, Plus, Search, Smile, Paperclip } from 'lucide-react';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ username }) => {
  ...
  const chats = [
    { id: 'general', name: 'General', unread: 0, lastMessage: 'Welcome to Swiper Chat!', isOnline: true },
    { id: 'secure', name: 'Secure Room', unread: 2, lastMessage: 'End-to-end encrypted', isOnline: true },
    { id: 'disappearing', name: 'Ghost Mode', unread: 0, lastMessage: 'Messages disappear in 10s', isOnline: false },
  ];
  ...
  const sendMessage = () => {
    ...
    toast({
      ...
      description: isDisappearingMode ? "Message will disappear in 10 seconds" : "Message saved with encryption",
    });
  ...
  const toggleDisappearingMode = () => {
    ...
    toast({
      title: isDisappearingMode ? "Standard mode enabled" : "Disappearing mode enabled",
      description: isDisappearingMode ? "Messages will be saved" : "Messages will disappear in 10 seconds",
    });
  };
  ...
  const toggleConsentMode = () => {
    ...
    toast({
      ...
      description: requireConsent ? "Messages saved automatically" : "Both users must consent to save messages",
    });
  };
        ...
        <div className="bg-black/40 backdrop-blur-xl border-b border-purple-500/30 p-4">
          ...
          <div className="flex items-center justify-between mt-3">
            <div className="flex items-center space-x-4 text-xs">
              ...
              <span className={`flex items-center px-2 py-1 rounded ${isDisappearingMode ? 'text-purple-300 bg-purple-500/10' : 'text-gray-500 bg-gray-500/10'}`}>
                <EyeOff className="h-3 w-3 mr-1" />
                {isDisappearingMode ? 'Disappearing Mode' : 'Standard Mode'}
              </span>
              <span className={`flex items-center px-2 py-1 rounded ${requireConsent ? 'text-blue-300 bg-blue-500/10' : 'text-gray-500 bg-gray-500/10'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="text-center text-gray-400 mt-20">
              ...
              <h3 className="text-xl font-semibold mb-2">Ultra-Secure Chat Initialized</h3>
              <p>Your messages are protected with military-grade encryption.</p>
              <p className="text-sm mt-2">🔐 Zero-knowledge architecture ensures complete privacy.</p>
        ...
        <div className="bg-black/40 backdrop-blur-xl border-t border-purple-500/30 p-4">
          ...
          <div className="flex items-center justify-between text-xs text-gray-400">
            <div className="flex items-center space-x-4">
              ...
              <span>🔥 {isDisappearingMode ? 'Disappears in 10s' : 'Saved with consent'}</span>
              <span>📱 {newMessage.length}/1000</span>
            </div>
            {isTyping && <span className="text-purple-300">You are typing...</span>}
          </div>
        </div>
      </div>
    </div>
